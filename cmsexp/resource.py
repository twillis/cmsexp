...
class PageContainer(BaseQuery):
    __model__ = m.Page
    __name__ = "page"

    def __getitem__(self, key):
        result = self.__qry__().filter_by(_slug=str(key).lower()).first()
        if result:
            return result
        else:
            return BaseQuery.__getitem__(self, key)

    def create(self, **data):
        record = self.__model__(**data)
        self._request.db.add(record)
        return record

    def list(self, **criteria):
        return self.__qry__().filter_by(**criteria)
...
class APIRoot(BaseResource):
    def __init__(self, request):
        ...
        self._create_child(PageContainer)
